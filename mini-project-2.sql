...
SELECT SB, State, AVG (Winner_Pts) AS avg_pts_winner, AVG (Loser_Pts) AS avg_pts_loser FROM superbowls GROUP BY SB HAVING avg_pts_winner > 30 and avg_pts_loser > 10;

SELECT Winner, SB, MAX (Winner_Pts) as max_pts_winner FROM superbowls;

SELECT Loser, SB, MAX (Loser_Pts) as max_pts_loser FROM superbowls;

SELECT Winner, SB, MIN (Winner_Pts) as min_pts_winner FROM superbowls;

SELECT Loser, SB, MIN (Loser_Pts) as min_pts_loser FROM superbowls;

SELECT City, COUNT(*) as number_of_times FROM superbowls GROUP BY City;

SELECT MVP, COUNT(MVP) FROM superbowls GROUP BY MVP;

SELECT Winner, COUNT(Winner) AS times_winner,
    CASE
        WHEN COUNT(Winner) == 1 THEN "One Time Champion"
        WHEN COUNT(Winner) >= 2 and COUNT(Winner) <= 3 THEN "2 or 3 Times Champions"
        WHEN COUNT(Winner) >= 4 THEN "4 or more Champions"
    END as "winner_table"
